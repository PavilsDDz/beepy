<?php 
   $texts = [];

    $texts['lv'] = [];
    $texts['en'] = [];
    $texts['ru'] = [];

// productstech spech//
$texts['lv']['techspech'] = 'Tehniskais apraksts';

$texts['en']['techspech'] = 'Tech spech';

$texts['ru']['techspech'] = 'Техническая спецификация';



$texts['lv']['emailN'] = 'E-pasts:';

$texts['en']['emailN'] = 'E-mail:';

$texts['ru']['emailN'] = 'Э-почта:';



$texts['lv']['subj'] = 'No kā:';

$texts['en']['subj'] = 'Subject:';

$texts['ru']['subj'] = 'Кто:';



$texts['lv']['mess'] = 'Ziņojums:';

$texts['en']['mess'] = 'Message:';

$texts['ru']['mess'] = 'Cообщение:';



$texts['lv']['mit'] = 'Nosūtīt';

$texts['en']['mit'] = 'Submit';

$texts['ru']['mit'] = 'Отправить';







// ----------------------------------------------->
// LV
//  CAR TYPE
    $texts['lv']['type'] = 'Virsbūves Tips';
    $texts['lv']['coupe'] = 'kupeja';
    $texts['lv']['hatchback'] = 'hečbeks';
    $texts['lv']['minivan'] = 'minivens';
    $texts['lv']['van'] = 'komerctransports';
    $texts['lv']['pickup'] = 'pikaps';
    $texts['lv']['sedan'] = 'sedans';
    $texts['lv']['unversal'] = 'unversālis';
    $texts['lv']['offroad'] = 'apvidus';
    $texts['lv']['sport'] = 'sporta';
    $texts['lv']['other'] = 'cits';

// Model
    $texts['lv']['model'] = 'Modelis';
    
// BRANDS
    $texts['lv']['brands'] = 'Markas';

// YEAR
    $texts['lv']['year'] = 'Gads';

    // MILLAGE
    $texts['lv']['millage'] = 'Nobraukums';

    $texts['lv']['km'] = 'km';
    $texts['lv']['from'] = 'No';
    $texts['lv']['to'] = 'Līdz';

// fUEAL TYPE
    $texts['lv']['fuel_type'] = 'Degviela';
    
    $texts['lv']['petrol'] = 'bendzīns';
    $texts['lv']['diesel'] = 'dīzelis';
    $texts['lv']['gas'] = 'gāze';
    $texts['lv']['electric'] = 'elektrība';

//  COLOR
    $texts['lv']['color'] = 'Krāsa';

    $texts['lv']['white'] = 'Balta';
    $texts['lv']['black'] = 'Melna';
    $texts['lv']['red'] = 'sarkana';
    $texts['lv']['yellow'] = 'dzeltena';
    $texts['lv']['green'] = 'zaļa';
    $texts['lv']['gray'] = 'pelēka';
    $texts['lv']['blue'] = 'zila';

//  PRICE
    $texts['lv']['price'] = 'Cena';

    $texts['lv']['euro'] = '€';
    $texts['lv']['from'] = 'No';
    $texts['lv']['to'] = 'Līdz';

// ENGINE CAPACITY
    $texts['lv']['engine_capacity'] = 'Motora Tilpums';

// TRANSMISSIONS
    $texts['lv']['transmission'] = 'Ātrumkārba';

    $texts['lv']['manual'] = 'mehāniskā';
    $texts['lv']['automatic'] = 'automātiskā';
    $texts['lv']['semi'] = 'pusautomātiskā';


//  REGISTRATION NUMBER
    $texts['lv']['RegNum'] = 'Reģistracijas numurs';

//  Technical Inspection:
    $texts['lv']['TechnicalInsp'] = 'Tehniskā apskate';

//  INFO:
    $texts['lv']['info'] = 'Papildus Informācija';
// -------------------------------------------------->
    // ENG
//  CAR TYPE
    $texts['en']['type'] = 'Car Type';
    $texts['en']['coupe'] = 'coupe';
    $texts['en']['hatchback'] = 'hatchback';
    $texts['en']['minivan'] = 'minivan';
    $texts['en']['van'] = 'van';
    $texts['en']['pickup'] = 'pickup';
    $texts['en']['sedan'] = 'sedan';
    $texts['en']['unversal'] = 'unversal';
    $texts['en']['offroad'] = 'offroad';
    $texts['en']['sport'] = 'sport';
    $texts['en']['other'] = 'other';

// MODEL
    $texts['en']['model'] = 'Model';

// BRANDS
    $texts['en']['brands'] = 'Brands';

// YEAR
    $texts['en']['year'] = 'Year:';

// MILLAGE
    $texts['en']['millage'] = 'Millage';

    $texts['en']['km'] = 'km';
    $texts['en']['from'] = 'From:';
    $texts['en']['to'] = 'To:';

// fUEAL TYPE
    $texts['en']['fuel_type'] = 'Fuel Type';
    
    $texts['en']['petrol'] = 'petrol';
    $texts['en']['diesel'] = 'diesel';
    $texts['en']['gas'] = 'gas';
    $texts['en']['electric'] = 'electric';

//  COLOR
    $texts['en']['color'] = 'Color';

    $texts['en']['white'] = 'white';
    $texts['en']['black'] = 'black';
    $texts['en']['red'] = 'red';
    $texts['en']['yellow'] = 'yellow';
    $texts['en']['green'] = 'green';
    $texts['en']['gray'] = 'gray';
    $texts['en']['blue'] = 'blue';

//  PRICE
    $texts['en']['price'] = 'Price';

    $texts['en']['euro'] = '€';
    $texts['en']['from'] = 'From:';
    $texts['en']['to'] = 'To:';

// ENGINE CAPACITY
    $texts['en']['engine_capacity'] = 'Engine Capacity';

// TRANSMISSIONS
    $texts['en']['transmission'] = 'Gear Box';

    $texts['en']['manual'] = 'manual';
    $texts['en']['automatic'] = 'automatic';
    $texts['en']['semi'] = 'semi';

//  REGISTRATION NUMBER
    $texts['en']['RegNum'] = 'Registration number';

//  Technical Inspection:
    $texts['en']['TechnicalInsp'] = 'Technical Inspection';

//  INFO:
    $texts['en']['info'] = 'Additional info';


// --------------------------------------------------------->
// RUS
//  CAR TYPE
    $texts['ru']['type'] = 'Тип кузова';
    $texts['ru']['coupe'] = 'купэ';
    $texts['ru']['hatchback'] = 'xэтчбек';
    $texts['ru']['minivan'] = 'минивэн';
    $texts['ru']['van'] = 'микроавтобус';
    $texts['ru']['pickup'] = 'пикап';
    $texts['ru']['sedan'] = 'cедан';
    $texts['ru']['unversal'] = 'универсал';
    $texts['ru']['offroad'] = 'внедорожник';
    $texts['ru']['sport'] = 'спорт';
    $texts['ru']['other'] = 'другой';

    // MODEL
    $texts['ru']['model'] = 'Модель';

// BRANDS
    $texts['ru']['brands'] = 'Марка';

// YEAR
    $texts['ru']['year'] = 'Год';

// MILLAGE
    $texts['ru']['millage'] = 'Показ одометра';

    $texts['ru']['km'] = 'км';
    $texts['ru']['from'] = 'От';
    $texts['ru']['to'] = 'До';

// fUEAL TYPE
    $texts['ru']['fuel_type'] = 'Топливо';

    $texts['ru']['petrol'] = 'бензин';
    $texts['ru']['diesel'] = 'дизель';
    $texts['ru']['gas'] = 'газ';
    $texts['ru']['electric'] = 'электричество';

//  COLOR
    $texts['ru']['color'] = 'Цвет';

    $texts['ru']['white'] = 'белый';
    $texts['ru']['black'] = 'чёрный';
    $texts['ru']['red'] = 'красный';
    $texts['ru']['yellow'] = 'жёлтый';
    $texts['ru']['green'] = 'зелёный';
    $texts['ru']['gray'] = 'серый';
    $texts['ru']['blue'] = 'синий';

//  PRICE
    $texts['ru']['price'] = 'Цена';

// ENGINE CAPACITY
    $texts['ru']['engine_capacity'] = 'Двигатель';

// TRANSMISSIONS
    $texts['ru']['transmission'] = 'Коробка передач';

    $texts['ru']['manual'] = 'механическая';
    $texts['ru']['automatic'] = 'автомат';
    $texts['ru']['semi'] = 'семи';

//  REGISTRATION NUMBER
    $texts['ru']['RegNum'] = 'Pегистрационный номер';

//  Technical Inspection:
    $texts['ru']['TechnicalInsp'] = 'Технический просмотр';
//  INFO:
    $texts['ru']['info'] = 'Дополнительная информация';


// ---------------------------------------------------------

// Extras LV 
    $texts['lv']['eq'] = 'Aprīkojums';
        $texts['lv']['hyd'] = 'Stūres pastiprinātājs';
        $texts['lv']['elBooster'] = 'Elektroniskais pastiprinātājs';
        $texts['lv']['condit'] = 'Kondicionieris';
        $texts['lv']['clicontr'] = 'Klimatu kontrole';
        $texts['lv']['salonFilt'] = 'Salona filtrs';
        $texts['lv']['onboardCom'] = 'Borta Dators';
        $texts['lv']['tirePre'] = 'Riepu spiediena kontroles mērītājs';
        $texts['lv']['parkSensor'] = 'Parkošanās sensori';
        $texts['lv']['rearViewCam'] = 'Atpakaļskata kamera';

    $texts['lv']['interior'] = 'Interjērs';
        $texts['lv']['leaInt'] = 'Ādas salons';
        $texts['lv']['handstand'] = 'Elkoņa atbalsts';
        $texts['lv']['tindWin'] = 'Tonētie stikli';
        $texts['lv']['ref'] = 'Ledusskapis';

$texts['lv']['SW'] = 'Stūre';
        $texts['lv']['addsteer'] = 'Stūres pastiprinātājs';
        $texts['lv']['eladd'] = 'Papildus elektroniskā stūrešana';
        $texts['lv']['multifuncwheel'] = 'Multifunkcionāla stūre';
        $texts['lv']['sportsteer'] = 'Sporta stūre';
        $texts['lv']['heatedwheel'] = 'Apsildāmā stūre';

$texts['lv']['seats'] = 'Krēsli';
        $texts['lv']['eladdseats'] = 'Ādas salons';
        $texts['lv']['heatedSeats'] = 'Elkoņa atbalsts';
        $texts['lv']['sportSeats'] = 'Tonētie stikli';
        $texts['lv']['rec'] = 'Ledusskapis';
        $texts['lv']['ventSeats'] = 'Sēdekļi ar ventilāciju';
        $texts['lv']['massageSeats'] = 'Elkoņa atbalsts';

$texts['lv']['lights'] = 'Gaismas';
        $texts['lv']['xenon'] = 'Xenon';
        $texts['lv']['bixen'] = 'Bixenon';
        $texts['lv']['led'] = 'LED';
        $texts['lv']['ledbrakled'] = 'Klimatu kontrole';
        $texts['lv']['foglight'] = 'Miglas Gaismas';
        $texts['lv']['lCleaner'] = 'Lampu tīrītāji';

$texts['lv']['mirrors'] = 'Spoguļi';
        $texts['lv']['eladdmir'] = 'Automāstiski regulējami spoguļi ';
        $texts['lv']['heatedMir'] = 'Apsildāmie spoguļi';
        $texts['lv']['sportaMir'] = 'Sporta Spoguļi';
        $texts['lv']['autoBend'] = 'Automātiskā spoguļu locīšana';

$texts['lv']['safety'] = 'Drošība';
        $texts['lv']['abs'] = 'ABS';
        $texts['lv']['ckey'] = 'Centrālā atslēga';
        $texts['lv']['alrm'] = 'Avarijas signāls';
        $texts['lv']['imob'] = 'imobilaizers';
        $texts['lv']['airB'] = 'Gaisa spilveni';
        $texts['lv']['esp'] = 'ESP';
        $texts['lv']['asr'] = 'ASR';
        $texts['lv']['marking'] = 'Marķēšana';

$texts['lv']['HIFI'] = 'HI - FI';
        $texts['lv']['fm/am'] = 'FM / AM';
        $texts['lv']['cd'] = 'CD';
        $texts['lv']['dvd'] = 'DVD';
        $texts['lv']['mp3'] = 'MP3';
        $texts['lv']['gps'] = 'GPS';
        $texts['lv']['bluet'] = 'Bluetooth';
        $texts['lv']['handfree'] = 'AUX';
        $texts['lv']['sub'] = 'Sabūfers';
        $texts['lv']['lcd'] = 'LCD televīzors';
        $texts['lv']['tv'] = 'Televīzors';

// Extras RUS
    $texts['ru']['eq'] = 'ОБОРУДОВАНИЕ';
        $texts['ru']['hyd'] = 'Гидроусилитель';
        $texts['ru']['elBooster'] = 'Электронный усилитель';
        $texts['ru']['condit'] = 'Kондиционер';
        $texts['ru']['clicontr'] = 'Контроль климата';
        $texts['ru']['salonFilt'] = 'Салоновый фильтр';
        $texts['ru']['onboardCom'] = 'Встроенный вычислитель';
        $texts['ru']['tirePre'] = 'Контроль давления шин';
        $texts['ru']['parkSensor'] = 'Датчик парковки';
        $texts['ru']['rearViewCam'] = 'Kамера заднего вида';

    $texts['ru']['interior'] = 'ИНТЕРЬЕР';
        $texts['ru']['leaInt'] = 'Кожаный салон';
        $texts['ru']['handstand'] = 'Подставка для рук';
        $texts['ru']['tindWin'] = 'Тонированные стекла';
        $texts['ru']['ref'] = 'холодильник';

$texts['ru']['SW'] = 'РУЛЬ';
        $texts['ru']['addsteer'] = 'Добавочное рулевое управление';
        $texts['ru']['eladd'] = 'Электронно добавочное рулевое колесо';
        $texts['ru']['multifuncwheel'] = 'Многофункциональное колесо';
        $texts['ru']['sportsteer'] = 'Спортивный руль';
        $texts['ru']['heatedwheel'] = 'Руль с подогревом';

$texts['ru']['seats'] = 'СИДЕНЬЯ';
        $texts['ru']['eladdseats'] = 'электронно снабженные сиденья';
        $texts['ru']['heatedSeats'] = 'сиденья с подогревом';
        $texts['ru']['sportSeats'] = 'Спортивные сиденья';
        $texts['ru']['rec'] = 'Recaro';
        $texts['ru']['ventSeats'] = 'Вентиляционные сиденья';
        $texts['ru']['massageSeats'] = 'Массажные сиденья';

$texts['ru']['lights'] = 'ОГНИ';
        $texts['ru']['xenon'] = 'ксенон';
        $texts['ru']['bixen'] = 'Би-ксенон';
        $texts['ru']['led'] = 'светодиод';
        $texts['ru']['ledbrakled'] = 'Светодиодные фонари';
        $texts['ru']['foglight'] = 'противотуманные огни';
        $texts['ru']['lCleaner'] = 'Светоотражатели';


$texts['ru']['mirrors'] = 'ЗЕРКАЛА';
        $texts['ru']['eladdmir'] = 'Электронно-приобретаемые зеркала';
        $texts['ru']['heatedMir'] = 'Подогреваемые зеркала';
        $texts['ru']['sportaMir'] = 'Спортивные зеркала';
        $texts['ru']['autoBend'] = 'Автоматический изгиб ';

$texts['ru']['safety'] = 'БЕЗОПАСНОСТЬ  ';
        $texts['ru']['abs'] = 'ABS';
        $texts['ru']['ckey'] = 'Центральный замок';
        $texts['ru']['alrm'] = 'Cигнал тревоги';
        $texts['ru']['imob'] = 'Иммобилайзер';
        $texts['ru']['airB'] = 'Подушки безопасности';
        $texts['ru']['esp'] = 'Gaisa spilveni';
        $texts['ru']['asr'] = 'ASR';
        $texts['ru']['marking'] = 'Mаркировка';

$texts['ru']['HIFI'] = 'HI - FI';
        $texts['ru']['fm/am'] = 'FM / AM';
        $texts['ru']['cd'] = 'CD';
        $texts['ru']['dvd'] = 'DVD';
        $texts['ru']['mp3'] = 'MP3';
        $texts['ru']['gps'] = 'GPS';
        $texts['ru']['bluet'] = 'Bluetooth';
        $texts['ru']['handfree'] = 'AUX';
        $texts['ru']['sub'] = 'Sabūfers';
        $texts['ru']['lcd'] = 'lcd televīzors';
        $texts['ru']['tv'] = 'Televīzors';

        // Extras ENG
    $texts['en']['eq'] = 'EQUIPMENT';
        $texts['en']['hyd'] = 'Hydrplic booster';
        $texts['en']['elBooster'] = 'Electronic booster';
        $texts['en']['condit'] = 'Conditioner';
        $texts['en']['clicontr'] = 'Climat control';
        $texts['en']['salonFilt'] = 'Salon ari filter';
        $texts['en']['onboardCom'] = 'Onboard computre';
        $texts['en']['tirePre'] = 'Tire presure control';
        $texts['en']['parkSensor'] = 'Parking sensor';
        $texts['en']['rearViewCam'] = 'Rear view camera';

    $texts['en']['interior'] = 'INTERIOR';
        $texts['en']['leaInt'] = 'Leather interior';
        $texts['en']['handstand'] = 'Hand stand';
        $texts['en']['tindWin'] = 'Tinted windows';
        $texts['en']['ref'] = 'Refrigerator';

$texts['en']['SW'] = 'STEERING WHEEL';
        $texts['en']['addsteer'] = 'Addaptable steering';
        $texts['en']['eladd'] = 'Electronicly addaptable steeringwheel';
        $texts['en']['multifuncwheel'] = 'Multi functional wheel';
        $texts['en']['sportsteer'] = 'Sports steering wheel';
        $texts['en']['heatedwheel'] = 'Heated steering wheel';

$texts['en']['seats'] = 'SEATS';
        $texts['en']['eladdseats'] = 'Electonicly addaptable seats';
        $texts['en']['heatedSeats'] = 'Heated seats';
        $texts['en']['sportSeats'] = 'Sport seats';
        $texts['en']['rec'] = 'Recaro';
        $texts['en']['ventSeats'] = 'Ventable seats';
        $texts['en']['massageSeats'] = 'Massage seats';

$texts['en']['lights'] = 'LIGHTS';
        $texts['en']['xenon'] = 'Xenon';
        $texts['en']['bixen'] = 'Bi xenon';
        $texts['en']['led'] = 'Led';
        $texts['en']['ledbrakled'] = 'Led braking lights';
        $texts['en']['foglight'] = 'Fog lights';
        $texts['en']['lCleaner'] = 'Light cleaners';


$texts['en']['mirrors'] = 'MIRRORS';
        $texts['en']['eladdmir'] = 'Electronicly addaptable mirrors';
        $texts['en']['heatedMir'] = 'Heated mirorrs';
        $texts['en']['sportaMir'] = 'Sports mirrors';
        $texts['en']['autoBend'] = 'Automatic bend';

$texts['en']['safety'] = 'SAFETY';
        $texts['en']['abs'] = 'ABS';
        $texts['en']['ckey'] = 'Central key';
        $texts['en']['alrm'] = 'Alarm';
        $texts['en']['imob'] = 'Imobilazer';
        $texts['en']['airB'] = 'Air bag';
        $texts['en']['esp'] = 'ESP';
        $texts['en']['asr'] = 'ASR';
        $texts['en']['marking'] = 'Marking';

$texts['en']['HIFI'] = 'HI - FI';
        $texts['en']['fm/am'] = 'FM / AM';
        $texts['en']['cd'] = 'CD';
        $texts['en']['dvd'] = 'DVD';
        $texts['en']['mp3'] = 'MP3';
        $texts['en']['gps'] = 'GPS';
        $texts['en']['bluet'] = 'Bluetooth';
        $texts['en']['handfree'] = 'AUX';
        $texts['en']['sub'] = 'Subwoofer';
        $texts['en']['lcd'] = 'LCD';
        $texts['en']['tv'] = 'TV';


?>