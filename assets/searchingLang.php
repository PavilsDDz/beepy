<?php 
   $texts = [];

    $texts['lv'] = [];
    $texts['en'] = [];
    $texts['ru'] = [];

// like lv, en, rus!!

$texts['lv']['likes'] = 'Patīk';

$texts['en']['likes'] = 'Likes';

$texts['ru']['likes'] = 'Нравится';

// ----------------------------------------------->
// LV
//  CAR TYPE
    $texts['lv']['type'] = 'Virsbūves Tips';
    $texts['lv']['coupe'] = 'kupeja';
    $texts['lv']['hatchback'] = 'hečbeks';
    $texts['lv']['minivan'] = 'minivens';
    $texts['lv']['van'] = 'komerctransports';
    $texts['lv']['pickup'] = 'pikaps';
    $texts['lv']['sedan'] = 'sedans';
    $texts['lv']['unversal'] = 'unversālis';
    $texts['lv']['offroad'] = 'apvidus';
    $texts['lv']['sport'] = 'sporta';
    $texts['lv']['other'] = 'cits';

// Model
    $texts['lv']['model'] = 'Modelis';
    
// BRANDS
    $texts['lv']['brands'] = 'Markas';

// YEAR
    $texts['lv']['year'] = 'Gads';

    // MILLAGE
    $texts['lv']['millage'] = 'Nobraukums';

    $texts['lv']['km'] = 'km';
    $texts['lv']['from'] = 'No';
    $texts['lv']['to'] = 'Līdz';

// fUEAL TYPE
    $texts['lv']['fuel_type'] = 'Degviela';
    
    $texts['lv']['petrol'] = 'bendzīns';
    $texts['lv']['diesel'] = 'dīzelis';
    $texts['lv']['gas'] = 'gāze';
    $texts['lv']['electric'] = 'elektrība';

//  COLOR
    $texts['lv']['color'] = 'Krāsa';

    $texts['lv']['white'] = 'Balta';
    $texts['lv']['black'] = 'Melna';
    $texts['lv']['red'] = 'sarkana';
    $texts['lv']['yellow'] = 'dzeltena';
    $texts['lv']['green'] = 'zaļa';
    $texts['lv']['gray'] = 'pelēka';
    $texts['lv']['blue'] = 'zila';

//  PRICE
    $texts['lv']['price'] = 'Cena';

    $texts['lv']['euro'] = '€';
    $texts['lv']['from'] = 'No';
    $texts['lv']['to'] = 'Līdz';

// ENGINE CAPACITY
    $texts['lv']['engine_capacity'] = 'Motora Tilpums';

// TRANSMISSIONS
    $texts['lv']['transmission'] = 'Ātrumkārba';

    $texts['lv']['manual'] = 'mehāniskā';
    $texts['lv']['automatic'] = 'automātiskā';
    $texts['en']['countinously '] = '...';
    $texts['lv']['semi'] = 'pusautomātiskā';

// SORT
    $texts['lv']['sort'] = 'Sortēt';

    $texts['lv']['sortpice'] = 'cena';
    $texts['lv']['sortmillage'] = 'nobraukums';
    $texts['lv']['sortyear'] = 'gads';

//  ORDER
    $texts['lv']['order'] = 'Kārtot pēc';

    $texts['lv']['orderto'] = 'No';
    $texts['lv']['orderfrom'] = 'Līdz';

//  SELECT FILTER
    $texts['lv']['selectf'] = 'Filtrēt';

// SEARCH
    $texts['lv']['search'] = 'Meklēt';

// -------------------------------------------------->
    // ENG
//  CAR TYPE
    $texts['en']['type'] = 'Car Type:';
    $texts['en']['coupe'] = 'coupe';
    $texts['en']['hatchback'] = 'hatchback';
    $texts['en']['minivan'] = 'minivan';
    $texts['en']['van'] = 'van';
    $texts['en']['pickup'] = 'pickup';
    $texts['en']['sedan'] = 'sedan';
    $texts['en']['unversal'] = 'unversal';
    $texts['en']['offroad'] = 'offroad';
    $texts['en']['sport'] = 'sport';
    $texts['en']['other'] = 'other';

// MODEL
    $texts['en']['model'] = 'Model:';

// BRANDS
    $texts['en']['brands'] = 'Brands:';

// YEAR
    $texts['en']['year'] = 'Year:';

// MILLAGE
    $texts['en']['millage'] = 'Millage:';

    $texts['en']['km'] = 'km';
    $texts['en']['from'] = 'From:';
    $texts['en']['to'] = 'To:';

// fUEAL TYPE
    $texts['en']['fuel_type'] = 'Fuel Type';
    
    $texts['en']['petrol'] = 'petrol';
    $texts['en']['diesel'] = 'diesel';
    $texts['en']['gas'] = 'gas';
    $texts['en']['electric'] = 'electric';

//  COLOR
    $texts['en']['color'] = 'Color';

    $texts['en']['white'] = 'white';
    $texts['en']['black'] = 'black';
    $texts['en']['red'] = 'red';
    $texts['en']['yellow'] = 'yellow';
    $texts['en']['green'] = 'green';
    $texts['en']['gray'] = 'gray';
    $texts['en']['blue'] = 'blue';

//  PRICE
    $texts['en']['price'] = 'Price';

    $texts['en']['euro'] = '€';
    $texts['en']['from'] = 'From:';
    $texts['en']['to'] = 'To:';

// ENGINE CAPACITY
    $texts['en']['engine_capacity'] = 'Engine Capacity';

// TRANSMISSIONS
    $texts['en']['transmission'] = 'Gear Box';

    $texts['en']['manual'] = 'manual';
    $texts['en']['automatic'] = 'automatic';
    $texts['en']['countinously '] = '...';
    $texts['en']['semi'] = 'semi';

// SORT
    $texts['en']['sort'] = 'Sort by';

    $texts['en']['sortpice'] = 'price';
    $texts['en']['sortmillage'] = 'millage';
    $texts['en']['sortyear'] = 'year';

//  ORDER
    $texts['en']['order'] = 'Order';

    $texts['en']['orderto'] = 'hight to low';
    $texts['en']['orderfrom'] = 'low to high';

//  SELECT FILTER
    $texts['en']['selectf'] = 'Select Filter';

// SEARCH
    $texts['en']['search'] = 'Search';

// --------------------------------------------------------->
// RUS
//  CAR TYPE
    $texts['ru']['type'] = 'Тип кузова';
    $texts['ru']['coupe'] = 'купэ';
    $texts['ru']['hatchback'] = 'xэтчбек';
    $texts['ru']['minivan'] = 'минивэн';
    $texts['ru']['van'] = 'микроавтобус';
    $texts['ru']['pickup'] = 'пикап';
    $texts['ru']['sedan'] = 'cедан';
    $texts['ru']['unversal'] = 'универсал';
    $texts['ru']['offroad'] = 'внедорожник';
    $texts['ru']['sport'] = 'спорт';
    $texts['ru']['other'] = 'другой';

    // MODEL
    $texts['ru']['model'] = 'Модель:';

// BRANDS
    $texts['ru']['brands'] = 'Марка';

// YEAR
    $texts['ru']['year'] = 'Год';

// MILLAGE
    $texts['ru']['millage'] = 'Показ одометра';

    $texts['ru']['km'] = 'км';
    $texts['ru']['from'] = 'От';
    $texts['ru']['to'] = 'До';

// fUEAL TYPE
    $texts['ru']['fuel_type'] = 'Топливо';

    $texts['ru']['petrol'] = 'бензин';
    $texts['ru']['diesel'] = 'дизель';
    $texts['ru']['gas'] = 'газ';
    $texts['ru']['electric'] = 'электричество';

//  COLOR
    $texts['ru']['color'] = 'Цвет';

    $texts['ru']['white'] = 'белый';
    $texts['ru']['black'] = 'чёрный';
    $texts['ru']['red'] = 'красный';
    $texts['ru']['yellow'] = 'жёлтый';
    $texts['ru']['green'] = 'зелёный';
    $texts['ru']['gray'] = 'серый';
    $texts['ru']['blue'] = 'синий';

//  PRICE
    $texts['ru']['price'] = 'Цена';

    $texts['ru']['euro'] = '€';
    $texts['ru']['from'] = 'От:';
    $texts['ru']['to'] = 'До:';

// ENGINE CAPACITY
    $texts['ru']['engine_capacity'] = 'Двигатель';

// TRANSMISSIONS
    $texts['ru']['transmission'] = 'Коробка передач';

    $texts['ru']['manual'] = 'механическая';
    $texts['ru']['automatic'] = 'автомат';
    $texts['ru']['semi'] = 'семи';

// SORT
    $texts['ru']['sort'] = 'сортировка';

    $texts['ru']['sortpice'] = 'цена';
    $texts['ru']['sortmillage'] = 'показ одометра';
    $texts['ru']['sortyear'] = 'год';

//  ORDER
    $texts['ru']['order'] = 'Cортировать';
    
    $texts['ru']['orderto'] = 'сортировать от самое большое ';
    $texts['ru']['orderfrom'] = 'сортировать от самое мeншая';

//  SELECT FILTER
    $texts['ru']['selectf'] = 'Выбрать фильтр';
// SEARCH
    $texts['ru']['search'] = 'Поиск';

// <!-- REGISTRĒŠANĀS-->

    $texts['lv']['signup_f'] = 'Ieiet caur FACEBOOK';
    $texts['lv']['fname'] = 'Vārds';
    $texts['lv']['lname'] = 'Uzvārds';
    $texts['lv']['uname'] = 'Lietotājs';
    $texts['lv']['email'] = 'E-Pasts';
    $texts['lv']['phone'] = 'Telefona numurs';
    $texts['lv']['country'] = 'Valsts';
    $texts['lv']['pass'] = 'Parole';
    $texts['lv']['re_pass'] = 'Parole atkārtoti';
    $texts['lv']['reg'] = 'Reģistrēties';

    $texts['lv']['nameErrorA'] = 'Lūdzu, ievadiet savu PILNO VĀRDU.';
    $texts['lv']['nameErrorB'] = 'Vārds nedrīkst saturēt mazāk kā 3 burtus.';
    $texts['lv']['nameErrorC'] = 'Name must contain alphabets and space.';

    $texts['lv']['lNameErrorA'] = 'Lūdzu, ievadiet savu PILNO UZVĀRDU.';
    $texts['lv']['lNameErrorB'] = 'Uzvārds nedrīkst saturēt mazāk kā 3 burtus.';
    $texts['lv']['lNameErrorC'] = 'Name must contain alphabets and space.';

    $texts['lv']['uNameErrorA'] = 'Lūdzu ievadiet savu LIETOTĀJ VĀRDU.';  
    $texts['lv']['uNameErrorB'] = 'Lietotāja vārds nedrīkst saturēt mazāk kā 3 burtus.';
    $texts['lv']['uNameErrorC'] = 'Atvainojiet, Jūsu LIETOTĀJ VĀRDS ir jau aizņemts!';

    $texts['lv']['emailErr'] = 'Atvainojiet, Jūsu E-PASTS mūsu sistēmā jau pastāv!';

    $texts['lv']['passErrorA'] = 'Lūdzu ievadiet savu paroli.';
    $texts['lv']['passErrorB'] = 'Jusu parole nedrīkst saturēt mazāk kā 6 simbolus.';

    $texts['lv']['passErrorC'] = 'Paroles nesakrīt!';

    $texts['lv']['succErr'] = 'Jūs esat veiksmīgi piereģistrējies!';
// ------------------------------------------------------>
    $texts['en']['signup_f'] = 'Sing Up with FACEBOOK';
    $texts['en']['fname'] = 'First Name';
    $texts['en']['lname'] = 'Last Name';
    $texts['en']['uname'] = 'User Name';
    $texts['en']['email'] = 'E-mail';
    $texts['en']['phone'] = 'Phone';
    $texts['en']['country'] = 'Country';
    $texts['en']['pass'] = 'password';
    $texts['en']['re_pass'] = 'Re-enter Password';
    $texts['en']['reg'] = 'Register';

    $texts['en']['nameErrorA'] = 'Please enter your full NAME.';
    $texts['en']['nameErrorB'] = 'Name must have atleat 3 characters.';
    $texts['en']['nameErrorC'] = 'Name must contain alphabets and space.';

    $texts['en']['lNameErrorA'] = 'Please enter your full LASTNAME.';
    $texts['en']['lNameErrorB'] = 'Name must have atleat 3 characters.';
    $texts['en']['lNameErrorC'] = 'Name must contain alphabets and space.';

    $texts['en']['uNameErrorA'] = 'Please enter your USERNAME.';  
    $texts['en']['uNameErrorB'] = 'Name must have atleat 3 characters.';
    $texts['en']['uNameErrorC'] = 'Sorry USERNAME already taken!';

    $texts['en']['emailErr'] = 'Sorry EMAIL id already taken!';

    $texts['en']['passErrorA'] = 'Please enter password.';
    $texts['en']['passErrorB'] = 'Password must have atleast 6 characters.';

    $texts['en']['passErrorC'] = 'Password does not match!';

    $texts['en']['succErr'] = 'You are successfully registered!';
// --------------------------------------------------------->
    $texts['ru']['signup_f'] = 'Вход чериз FACEBOOK';
    $texts['ru']['fname'] = 'Имя';
    $texts['ru']['lname'] = 'Фамилия';
    $texts['ru']['uname'] = 'Пользователь';
    $texts['ru']['email'] = 'Злектронойпочтa';
    $texts['ru']['phone'] = 'Номер телефона';
    $texts['ru']['country'] = 'Страна';
    $texts['ru']['pass'] = 'Пароль';
    $texts['ru']['re_pass'] = 'Повтор пароля';
    $texts['ru']['reg'] = 'Мой профиль';

    $texts['ru']['nameErrorA'] = 'Пожалуйста, введите ваше полное имя.';
    $texts['ru']['nameErrorB'] = 'Имя должно иметь 3 символа.';
    $texts['ru']['nameErrorC'] = 'Имя должно содержать буквы и пробел.';

    $texts['ru']['lNameErrorA'] = 'Введите вашу ФАМИЛИЮ.';
    $texts['ru']['lNameErrorB'] = 'ФАМИЛИЯ должно иметь 3 символа.';
    $texts['ru']['lNameErrorC'] = 'ФАМИЛИЯ должно содержать буквы и пробел.';

    $texts['ru']['uNameErrorA'] = 'Пожалуйста, введите Ваш логин.';  
    $texts['ru']['uNameErrorB'] = 'Имя должно иметь 3 символа.';
    $texts['ru']['uNameErrorC'] = 'К сожалению, такое имя пользователя уже занято!';

    $texts['ru']['emailErr'] = 'Извините электронная уже занята!';

    $texts['ru']['passErrorA'] = 'Пожалуйста введите пароль.';
    $texts['ru']['passErrorB'] = 'Пароль должен содержать не менее 6 символов.';

    $texts['ru']['passErrorC'] = 'Пароль не подходит!';

    $texts['ru']['succErr'] = 'Вы успешно зарегистрированы!';
    

?>

<?php

    $texts['lv']['findcar'] = "Meklēt Mašīnu";

    $texts['en']['findcar'] = "Find A Car";

    $texts['ru']['findcar'] = "Поиск Автомобилей";


    $texts['lv']['choose'] = "Izvēlēties brandu";

    $texts['en']['choose'] = "Choose a brand";

    $texts['ru']['choose'] = "Bыбрать бренд";



    $texts['lv']['chooseThebest'] = "Atrodiet īsto mašīnu <br> priekš sevis";

    $texts['en']['chooseThebest'] = "Find the right <br> car for you";

    $texts['ru']['chooseThebest'] = "Bыбрать правильную <br> машину для себя";


    
    $texts['lv']['why'] = "Kādēļ Beepy?";

    $texts['en']['why'] = "Why Beepy?";

    $texts['ru']['why'] = "Почему Beepy?";


    $texts['lv']['present'] = "Skatiet mūsu prezentāciju";

    $texts['en']['present'] = "See our presentation";

    $texts['ru']['present'] = "Cмотрите нашу презентацию";

    $texts['lv']['search'] = "Meklēt";

    $texts['en']['search'] = "Search";

    $texts['ru']['search'] = "Поиск";



// lv
    $texts['lv']['text1name'] = "Viss vienuviet";
        $texts['lv']['text4'] = "Auto; moto; kravas auto; serviss. Beepy ir revolucionārs digitāls auto tirdzniecības portāls, kas saviem lietotājiem piedāvā vispilnīgāko un vieglāk saprotamo auto iegādes pieredzi Baltijā.";

    $texts['lv']['text2name'] = "Vienmēr saskarē ar jauno";
        $texts['lv']['text5'] = "Ar nelietoto mašīnu uzbūves tipiem iespējams iepazīties 'Auto skatē', kur demonstrēti visi portāla piedāvāto marku modeļi, ļaujot potenciālajam klientam atrast sev atbilstošāko, nepametot mājas komfortu.";

    $texts['lv']['text3name'] = "Jauna auto tirdzniecības sistēma";
        $texts['lv']['text6'] = "Kompānijai izanalizējot un publicējot informāciju par aktuālajiem cenu rādītājiem, nodrošināti nepieciešamie parametri godīgiem darījumiem un savstarpēja uzticība starp auto pircējiem un pārdevējiem.";

// en
    $texts['en']['text1name'] = "All in one-auto; motto; trucks; service.";
        $texts['en']['text4'] = "Beepy is a revolutionary digital car sales portal what offers the most complete for users and easily understandable car purchase experience in the Baltics.";

    $texts['en']['text2name'] = "Always in contact with the new";
        $texts['en']['text5'] = "With an unused car design types you may click on 'Auto show' where are all the brand models support portal, allowing potential client find a best fit, without leaving home comfort.";

    $texts['en']['text3name'] = "A new car trading system";
        $texts['en']['text6'] = "To the analysis and publication of information on the real price indicators, the necessary parameters fair transactions and mutual trust between buyers and sellers of car.";

//  ru
    $texts['ru']['text1name'] = "Все в одном месте";
        $texts['ru']['text4'] = "Mашины; мотоциклы; грузовые автомобили; сервис. Beepy является революционным порталом в дигатальной торговле, который предлагает своим пользователям облегченное приобретение опыта в покупке автомобилей в Балтии.";

    $texts['ru']['text2name'] = "Всегда соприкосновение с новым";
        $texts['ru']['text5'] = "С не используемыми типами машин можно ознакомиться 'Просмотр машин', где будут демонстрироваться все марки и модели предлагаемые порталом, позволяющего потенциальному клиенту найти себе более соответствующий автомобиль, не покидая домашний комфорт.";

    $texts['ru']['text3name'] = "Новая система торговли";
        $texts['ru']['text6'] = "Kомпании, проанализировав и публикуя информацию о самых актуальных исходя из роста цен, обеспечены необходимые параметры для честных сделок и взаимодоверия между покупателями и продавцами автомобилей.";


    $texts['lv']['blog'] = "Blogs";

    $texts['en']['blog'] = "Blog";

    $texts['ru']['blog'] = "Блог";



// lv
    $texts['lv']['text4name'] = "Plašākās auto darījumu iespējas Baltijā";
        $texts['lv']['text1'] = "Mājaslapa piedāvā atsevišķas sadaļas jauniem un lietotiem auto. Plašs darījumu iespēju klāsts nodrošina vislielākās auto interesentu piesaistes iespējas Baltijā.";

    $texts['lv']['text5name'] = "Tavs ideālais auto klikšķa attālumā";
        $texts['lv']['text2'] = "Lietotāju ērtībai pieejams “Viedais meklētājs”, kas ļauj atlasīt klientam vispiemērotākos auto modeļus, atbilstoši nepieciešamībai, vēlmēm un iespējām. Papildus piedāvāti modeļu apraksti ar iespēja tos salīdzināt.";

    $texts['lv']['text6name'] = "Ērtāk, Ātrāk un Vienkāršāk";
        $texts['lv']['text3'] = "Ērtāk – online katalogs ar sadaļu katrai jauno auto markai. Iespēja prezentēt savu marku starp Latvijā vadošajiem auto dīleriem. Ātrāk – ļauj auto interesentiem nākt pie tevis. Piesaisti klientus, kas jau ir ieinteresēti auto iegādē. Vienkāršāk – ietaupi laiku un   līdzekļus savas mājas lapas administrēšanā. Izstādi piedāvājumu tam paredzētajā sagatavē un sāc tirgoties jau šodien.";

// en
    $texts['en']['text4name'] = "The wider car business opportunities in Baltics";
        $texts['en']['text1'] = "The site offers some new and used cars. Provides a wide range of business opportunities in the lead car opportunities in Baltics.";

    $texts['en']['text5name'] = "Your ideal car one click away";
        $texts['en']['text2'] = "User convenience available “Smart seeker”, which allows you to select the most appropriate car models, according to customer needs, desires and options. In addition to the proposed model descriptions with the opportunity to compare them.
";

    $texts['en']['text6name'] = "Comfortable, Faster, Easier";
        $texts['en']['text3'] = "Comfortable – online catalog with each new car brand. Present your brand among the leading car dealers in Latvia. Faster – allows car enthusiasts to come to you. To customers who are already interested in purchasing the car.Easier – saving time and money administering your home page. The exhibition will offer in blank and begin to trade today.";

//  ru
    $texts['ru']['text4name'] = "Обширнейшие возможности авто сделок в Балтии";
        $texts['ru']['text1'] = "Mашины; мотоциклы; грузовые автомобили; сервис. Beepy является революционным порталом в дигатальной торговле, который предлагает своим пользователям облегченное приобретение опыта в покупке автомобилей в Балтии.";

    $texts['ru']['text5name'] = "Твой идеальный автомобиль находился на расстоянии нажатия";
        $texts['ru']['text2'] = "С не используемыми типами машин можно ознакомиться 'Просмотр машин', где будут демонстрироваться все марки и модели предлагаемые порталом, позволяющего потенциальному клиенту найти себе более соответствующий автомобиль, не покидая домашний комфорт.";

    $texts['ru']['text6name'] = "Гораздо удобнее, Скорее и Проще";
        $texts['ru']['text3'] = "Гораздо удобнее – для каждой марки машины свой каталог с данным разделом. Возможность презентовать свою марку, между Латвийскими ведущими автомобильными дилерами. Скорее – позволяет всем желающим автомобиля прийти к тебе. Привлечение клиентов, которые уже заинтересованы приобретением автомобиля. Проще – экономь время и средства для администрирования своей домашней страницы. Выставь предложение и начинай торговаться уже сегодня.";

?>